	...
	if (isServer)
	{
        ...
        for (int test = 0; test < testCnt; test++) {
			...
			for (int j = 0; j < 12; j++) {
				...
				// Receive one msg to sync with the client
				rc = zmq_recv(socket, &msg, 0);
				if (rc != 0) {
					printf("Error on first recv: %s\n", zmq_strerror(errno));
					return -1;
				}
				if (zmq_msg_size(&msg) != msgSize+4) {
					printf("Message(0) of incorrect size received: %u != %u\n", (unsigned int)zmq_msg_size(&msg), (unsigned int)msgSize);
					return -1;
				}
				crc = crc32(0L, (Bytef*)zmq_msg_data(&msg), msgSize);
				...
				printf("%u,%.2f\n", (unsigned int)msgSize, (double) megabits);
			}
        }
	}
	else

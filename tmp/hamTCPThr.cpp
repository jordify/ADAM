...
void* Connection::receive(int dataLength, double timeout, clock_t start) throw (Except) {
// 	unsigned int dataLengthgth = 0; 
	// Grab header
	unsigned int n = 0;
// 	while (1) { 
// 		unsigned char *tempData = (unsigned char*) &dataLengthgth; 
        ...
        for (int test = 0; test < testCnt; test++) {
			...
			for (int j = 0; j < 10; j++) {
				...
				for (int i = 0; i < messageCount; i++) {
					int msgSizeRec = 0;
					char* recvMsg = (char*) pipe->receive(msgSize);
